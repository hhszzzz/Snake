...
//
//        for (Obstacle ob : obs) { // 遍历每个障碍物
//            for (Node node : ob.nodes) { // 每个障碍物的每个节点
//                if (node.x * 20 == x && node.y * 20 + 2 == y) {
//                    ob.nodes.remove(node); // 移除该节点
//                    overlap.remove(node); // 在 overlap 中同时移除节点
//                    isLive = false;
//                    is_collision = true;
//                    break;
//                }
